...
@_example SGDClassifier "model = SGDClassifier(epochs=20, batch_size=64)"
@_example SGDRegressor "model = SGDRegressor(epochs=20, batch_size=64)"
@_example MARSRegressor "model = MARSRegressor(max_terms=21, max_degree=2)"
@_example PartialLeastSquaresRegression "model = PartialLeastSquaresRegression(n_components=3)"
@_example PCA "model = PCA(n_components=2)"
@_example TruncatedSVD "model = TruncatedSVD(n_components=2)"
@_example NMF "model = NMF(n_components=4)"
@_example RandomProjection "model = RandomProjection(n_components=20, distribution=:sparse)"
@_example FastICA "model = FastICA(n_components=4)"
@_example FeatureAgglomeration "model = FeatureAgglomeration(n_clusters=10)"
@_example KMeans "model = KMeans(n_clusters=3)"
@_example DBSCAN "model = DBSCAN(radius=0.5, min_neighbors=5)"
@_example AgglomerativeClustering "model = AgglomerativeClustering(n_clusters=3, linkage=:average)"
